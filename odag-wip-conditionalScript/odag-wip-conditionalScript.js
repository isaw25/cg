			...
			$scope.processIncludeFiles=function(script){
			  ...
			  return new Promise(function(resolve,reject){
				$scope.getVarByName("vDatabase",function(database){
				  ...
				  var includeScriptPathtemp = script.match(/<OnDemandApp_ConditionalScript>[\w\W]*?(?=<\/OnDemandApp_ConditionalScript>)/gim)[0];
				  var conditionalScriptContent=includeScriptPathtemp.substr(30).slice(1,-1);
				  //console.log("getting include file path");
				 console.log("Conditional script content="+conditionalScriptContent);

				  //if db is CPRD or JMDC , skip 
				  if(database==="CPRD" || database==="JMDC"){
				  
					script = script.replace(includeScriptPathtemp+"</OnDemandApp_ConditionalScript>","");

				  }else
				  {
				
					  //include payer script 
					  script = script.replace(includeScriptPathtemp+"</OnDemandApp_ConditionalScript>",conditionalScriptContent);
