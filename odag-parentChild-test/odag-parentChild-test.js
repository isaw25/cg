...
function ( appGenHtml, qvangular, $, RPCSession ) {
		...
		definition: {
			...
			items: {
              ...
              type: "string"
						...
						templateName: {
							ref: "advanced.templateName",
              translation: "Template App Name",
              type: "string"
						},
                      delSheet: {
        ...
        show: true
			...
			$scope.createApp = function(){
                ...
                session.rpc({handle: -1, method: "GetDocList", params:[]}).then(function(response1){
					var i=0; 
                  do{
                   doclist=response1.result.qDocList;
                   ...
                   if(!(PrevId))
				   {
					...
					session.rpc({handle: -1, method: "CreateApp", params: [appName+"_"+uName]}).then(function(response){
						if(response.result.qSuccess==true){
						 ...
						 var CopyFromAppName=$scope.templateName;
						 var temp=doclist.find(function(e){return e.qDocName==CopyFromAppName;});
						 if(temp){
						 CopyFromappId=temp.qDocId;
						 }else{
							$scope.logError("Unable to find template App :"+CopyFromAppName);
							return;
						 }
						 
						 
                          session.rpc({handle: -1, method: "CopyApp", params: [newAppId, CopyFromappId,[]]}).then(function(response){
								//open the new app
                ...
                if(script2)
				{
                  ...
                  {						   
					...
					if(openSheetScript && sheetContent)
					{
					 ...
					 $scope.processIncludeFiles(script1).then(function(script1){
			...
			//better way is to just keep the script within an <OnDemandApp_ConditionalScript> [script content]</OnDemandApp_ConditionalScript>
			//and based on db, replace it with the content or empty string ("").
			//no need of external include file
			
			$scope.processIncludeFiles=function(script){
					 
			  return new Promise(function(resolve,reject){
				$scope.getVarByName("vDatabase",function(database){
				  console.log("dbname="+database);

				  var includeScriptPathtemp = script.match(/<OnDemandApp_IncludeScript>[\w\W]*?(?=<\/OnDemandApp_IncludeScript>)/gim);
				  if(!includeScriptPathtemp){
				  //no match, just return
				  resolve(script);
				  }
				  includeScriptPathtemp=includeScriptPathtemp[0];
				  var includeScriptPath=includeScriptPathtemp.substr(27).slice(1,-1);
				  console.log("getting include file path");
				  console.log("Include file path="+includeScriptPath);

				  //if db is CPRD or JMDC , skip 
				  if(database==="CPRD" || database==="JMDC"){
					script = script.replace(includeScriptPathtemp+"</OnDemandApp_IncludeScript>","");
					resolve(script);
				  }else
				  {
				  
					//get the include file, and include the content in the script
					$scope.getIncludeFileContent(includeScriptPath,function(content){
				
					  console.log("content:"+content);

					  if(!content){content="";}
					  //include payer script 
					  script = script.replace(includeScriptPathtemp+"</OnDemandApp_IncludeScript>",content);
					  resolve(script);
  ...
  $scope.getVariableData = function(vStringData,vItr,vChk,cf2Data,cf2Count,callbackFn)
  {
	...
	{callbackFn.call(null,vStringData);}
			...
			$scope.getVarByName=function(varName,callbackFn){
		
			 session.rpc({handle: handle, method: "GetVariableByName", params:[varName]}).then(function(response1)
             {
			 	var vHandle=response1.result.qReturn.qHandle;
			 
			 	session.rpc({handle: vHandle, method: "GetLayout", params:[]}).then(function(response2){
					callbackFn.call(null,response2.result.qLayout.qText);	
				});
			...
			$scope.getIncludeFileContent=function(includefilePath,callbackFn){
			
			
				session.rpc({handle: handle, method: "GetIncludeFileContent", params : [includefilePath] }).then(function(response){
				
					var script=response.result.qContent;
				
					callbackFn.call(null,script);
		...
		paint: function ($element, layout) {
			...
			this.$scope.templateName = layout.advanced.templateName;
			...
			//dont change the view when processing-wasi
			if(this.$scope.rowCount && this.$scope.limit && this.$scope.view!="processing"){
